...
float LMotorController::map(float x, float in_min, float in_max, float out_min, float out_max)
{
...
void LMotorController::move(int leftSpeed, int rightSpeed, int minAbsSpeed)
{
    if (rightSpeed < 0)
    {
        rightSpeed = fminf(rightSpeed, -1*minAbsSpeed);
        rightSpeed = fmaxf(rightSpeed, -255);
    }
    else if (rightSpeed > 0)
    {
        rightSpeed = fmaxf(rightSpeed, minAbsSpeed);
        rightSpeed = fminf(rightSpeed, 255);
    }
    
    int realRightSpeed = LMotorController::map(fabsf(rightSpeed), 0, 255, minAbsSpeed, 255);

    if (leftSpeed < 0)
    {
        leftSpeed = fminf(leftSpeed, -1*minAbsSpeed);
        leftSpeed = fmaxf(leftSpeed, -255);
    }
    else if (leftSpeed > 0)
    {
        leftSpeed = fmaxf(leftSpeed, minAbsSpeed);
        leftSpeed = fminf(leftSpeed, 255);
    }
    
    int realLeftSpeed = LMotorController::map(fabsf(leftSpeed), 0, 255, minAbsSpeed, 255);
...
void LMotorController::move(int speed, int minAbsSpeed)
{
    ...
    if (speed < 0)
    {
        ...
        speed = fminf(speed, -1*minAbsSpeed);
        speed = fmaxf(speed, -255);
    }
    else
    {
        speed = fmaxf(speed, minAbsSpeed);
        speed = fminf(speed, 255);
    }
    
    if (speed == _currentSpeed) return;
    
    int realSpeed = fmaxf(minAbsSpeed, fabsf(speed));
...
void LMotorController::move(int speed)
{
    ...
    digitalWrite(_in3, speed > 0 ? HIGH : LOW);
    digitalWrite(_in4, speed > 0 ? LOW : HIGH);
    analogWrite(_ena, fabsf(speed) * _motorAConst);
    analogWrite(_enb, fabsf(speed) * _motorBConst);
